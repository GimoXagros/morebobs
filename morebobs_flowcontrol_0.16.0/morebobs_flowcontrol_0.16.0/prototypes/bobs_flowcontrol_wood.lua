local bobs_overflow_pipe_names = {} do
	...
	for key,value in pairs(n) do
		...
		if string.sub(key,2)=="s" then  -- Straight pipe
			new_pipes[value].item.subgroup = "energy-pipe-distribution"
		elseif string.sub(key,2)=="j" then  -- Junction pipe
			new_pipes[value].item.subgroup = "energy-pipe-distribution"
		elseif string.sub(key,2)=="e" then  -- Elbow pipe
			new_pipes[value].item.subgroup = "energy-pipe-distribution"
	...
	do -- These match the fluid areas listed in Bio_Industries_2.6.14\prototypes\entity\pipes.lua
	...
	do -- These match the fluid areas listed in Bio_Industries_2.6.14\prototypes\entity\pipes.lua
		new_pipes[n.qs].item.order = "a[pipe]-a[pipe]-0-3"
		new_pipes[n.qj].item.order = "a[pipe]-a[pipe]-0-4"
		new_pipes[n.qe].item.order = "a[pipe]-a[pipe]-0-5"
	end
	...
	data.raw["item"]["bi-wood-pipe"].order = "a[pipe]-a[pipe]-0-1"
	data.raw["item"]["bi-pipe-to-ground-wood"].order = "a[pipe]-a[pipe]-0-2"
	data.raw.item["pipe"].order = "a[pipe]-a[pipe]-1-1"
	data.raw.item["pipe-to-ground"].order = "a[pipe]-a[pipe]-1-2"
	data.raw.item["pipe-straight"].order = "a[pipe]-a[pipe]-1-3"
	data.raw.item["pipe-junction"].order = "a[pipe]-a[pipe]-1-4"
	data.raw.item["pipe-elbow"].order = "a[pipe]-a[pipe]-1-5"
	...
	if bobmods then
		if bobmods.logistics then
			...
			data.raw["item"]["pipe-bi-wood-straight"].subgroup = "flow-control-1"
			data.raw["item"]["pipe-bi-wood-junction"].subgroup = "flow-control-2"
			data.raw["item"]["pipe-bi-wood-elbow"].subgroup = "flow-control-3"
			...
			if bobmods.plates then
				...
				data.raw["item"]["bi-pipe-to-ground-wood"].order = "a[pipe]-b[pipe-to-ground]"
