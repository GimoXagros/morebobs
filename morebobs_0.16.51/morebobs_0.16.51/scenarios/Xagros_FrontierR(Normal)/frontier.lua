...
local ore_base_quantity	  = 400  --base ore quantity, everything is scaled up from this
	...
	if event.area.left_top.x <= deathworld_boundary and event.area.right_bottom.x >= deathworld_boundary then
		...
		for dy = 0, 31 do
			...
			for dx = 0, 4 do
				...
				if event.surface.can_place_entity{name = "stone-wall", position = {px, py}, force = "neutral"} then
					local e = event.surface.create_entity{name = "stone-wall", position = {px, py}, force = "neutral"}
					e.minable = false
				end
			end
		end
